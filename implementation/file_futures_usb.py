...
DEFS_DIR = "/media/passport/defs/"  # répertoire sur disque amovible
REDEFS_DIR = "/media/passport/redefs/"  # répertoire sur disque amovible
...
def local_define(word):
    ...
    with open(f"{DEFS_DIR}/{word}.txt") as fp:
        definition = fp.readline()
    with open(f"{REDEFS_DIR}/{word}.txt", "w") as fp:
        fp.write(definition)
